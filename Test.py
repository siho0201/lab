...
for i in range(-5,6): # all snr dataset
            ...
            random_number = randint(51,499)
            for j in range(len(signal_name)):
                label.extend(tx[signal_name[j]+'_tx'][random_number:random_number+1])
                input.extend(rx[signal_name[j]][random_number:random_number+1])
...
check_output_image(input_image, output_image,answer_image)
print('----------------------finish----------------------')